...
export default new SlashCommand({
  ...
  callback: async (logger, client, interaction) => {
    ...
    Database.execute(
      'DELETE FROM `customer_developers` WHERE `tbxid` = ? AND `discord_id` = ?',
      [purchase.tbxid, developer.id]
    );
    ...
    logger.success(`${developer.username} (${developer.id}) was removed from purchase ${purchase.tbxid} as developer by ${user.username} (${user.id})`);
  }
});
